jQuery.namespace("jira.widget.favourite");

jira.widget.favourite.NewPicker = function(options) {
...
jira.widget.favourite.Picker = function(options) {
    ...
    addUndoAction = function() {
        if(options["undoText"] !== undefined) {
            ...
            var undoDiv = AJS.$("<div/>").html(AJS.format(options["undoText"],undoEntityName));
            var favNode = AJS.$("#" + options["removeId"]);
            undoDiv.find("a").attr("id","fav_undo_a_" + options["removeId"]).click(function(e) {
                e.preventDefault();
    ...
    invalidateFiltersList = function() {
        if (relatedDropdown){
            if (jira.widget.dropdown[relatedDropdown]){
                jira.widget.dropdown[relatedDropdown].resetCache();
            }
        }
    },
...
jira.widget.favourite.Picker.init = function(parent) {
    AJS.$(".favourite-params", parent).each(function() {
        ...
        if(params["remote"] === "false") {
            ...
            jira.widget.favourite.NewPicker(params);
        } else {
            jira.widget.favourite.Picker(params);
        }
...
AJS.$(function() {
    jira.widget.favourite.Picker.init();
});
